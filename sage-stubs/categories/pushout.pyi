from typing import Self

from sage.categories.morphism import Morphism
from sage.structure.element import Element
from sage.structure.parent import Parent

class ConstructionFunctor:
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def pushout(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...
    def commutes(self, other: ConstructionFunctor) -> bool: ...
    def expand(self) -> list[ConstructionFunctor]: ...
    def common_base(
        self,
        other_functor: ConstructionFunctor,
        self_bases: object,
        other_bases: object,
    ) -> Parent: ...

class CompositeConstructionFunctor(ConstructionFunctor):
    def __init__(self, *args: object) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def expand(self) -> list[ConstructionFunctor]: ...

class IdentityConstructionFunctor(ConstructionFunctor):
    def __init__(self) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...

class MultivariateConstructionFunctor(ConstructionFunctor):
    def common_base(
        self,
        other_functor: ConstructionFunctor,
        self_bases: object,
        other_bases: object,
    ) -> Parent: ...

class PolynomialFunctor(ConstructionFunctor):
    def __init__(
        self,
        var: object,
        multi_variate: object = ...,
        sparse: object = ...,
        implementation: object = ...,
    ) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class MultiPolynomialFunctor(ConstructionFunctor):
    def __init__(self, vars: object, term_order: object) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...
    def expand(self) -> list[ConstructionFunctor]: ...

class InfinitePolynomialFunctor(ConstructionFunctor):
    def __init__(self, gens: object, order: object) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...
    def expand(self) -> list[ConstructionFunctor]: ...

class MatrixFunctor(ConstructionFunctor):
    def __init__(self, nrows: object, ncols: object, sparse: object = ...) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class LaurentPolynomialFunctor(ConstructionFunctor):
    def __init__(self, var: object, multi_variate: object = ...) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class VectorFunctor(ConstructionFunctor):
    def __init__(
        self,
        n: object,
        is_ambient: object = ...,
        sparse: object = ...,
        inner_product_matrix: object = ...,
    ) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class SubspaceFunctor(ConstructionFunctor):
    def __init__(self, basis: object) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class FractionField(ConstructionFunctor):
    def __init__(self) -> None: ...

class CompletionFunctor(ConstructionFunctor):
    type: None

    def __init__(self, p: object, prec: object, extras: object = ...) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...
    def commutes(self, other: ConstructionFunctor) -> bool: ...

class QuotientFunctor(ConstructionFunctor):
    names: None

    def __init__(self, I: object, names: object = ...) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class AlgebraicExtensionFunctor(ConstructionFunctor):
    def __init__(self, poly: object, name: object, embedding: object = ...) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...
    def __mul__(self, other: ConstructionFunctor) -> ConstructionFunctor: ...
    def expand(self) -> list[ConstructionFunctor]: ...

class AlgebraicClosureFunctor(ConstructionFunctor):
    def __init__(self) -> None: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class PermutationGroupFunctor(ConstructionFunctor):
    def __init__(self, gens: object, domain: object = ...) -> None: ...
    def __call__(self, R: object) -> Parent: ...
    def gens(self) -> tuple[Element, ...]: ...
    def merge(self, other: ConstructionFunctor) -> Self | None: ...

class EquivariantSubobjectConstructionFunctor(ConstructionFunctor):
    def __init__(self, group: object, semi: object) -> None: ...

class BlackBoxConstructionFunctor(ConstructionFunctor):
    def __init__(self, func: object) -> None: ...
    def __eq__(self, other: object) -> bool: ...
    def __ne__(self, other: object) -> bool: ...

def pushout(self, S: Parent) -> Parent: ...
def pushout_lattice(self, S: Parent) -> tuple[Morphism, Morphism]: ...
def construction_tower(self) -> list[tuple[ConstructionFunctor | None, Parent]]: ...
def expand_tower(self) -> list[tuple[ConstructionFunctor | None, Parent]]: ...
def type_to_parent(self) -> Parent: ...
