from collections.abc import Iterable, Sequence

from sage.structure.element import Element

from ...categories.homset import Homset
from ...dynamics.arithmetic_dynamics.projective_ds import DynamicalSystem_projective
from ...rings.ideal import Ideal_generic
from ...rings.integer import Integer
from ...rings.polynomial.multi_polynomial_ideal import MPolynomialIdeal
from ...rings.polynomial.polynomial_element import Polynomial
from ...rings.ring import Ring
from ...structure.element import RingElement
from ..affine.affine_space import AffineSpace_generic
from ..affine.affine_subscheme import AlgebraicScheme_subscheme_affine
from ..generic.algebraic_scheme import AlgebraicScheme_subscheme
from .projective_morphism import SchemeMorphism_polynomial_projective_subscheme_field
from .projective_point import SchemeMorphism_point_projective_ring
from .projective_space import ProjectiveSpace_field

type _PointLike = SchemeMorphism_point_projective_ring | Sequence[RingElement]
type _PolynomialInput = Polynomial | Ideal_generic | Iterable[Polynomial]

class AlgebraicScheme_subscheme_projective(AlgebraicScheme_subscheme):
    def __init__(
        self, A: ProjectiveSpace_field, polynomials: _PolynomialInput
    ) -> None: ...
    def point(
        self, v: _PointLike, check: bool = True
    ) -> SchemeMorphism_point_projective_ring: ...
    def _morphism(
        self,
        parent: Homset[
            AlgebraicScheme_subscheme_projective, AlgebraicScheme_subscheme_projective
        ],
        polys: Sequence[Polynomial],
        check: bool = True,
    ) -> SchemeMorphism_polynomial_projective_subscheme_field: ...
    def dimension(self) -> Integer: ...
    def affine_patch(
        self, i: Integer, AA: AffineSpace_generic | None = None
    ) -> AlgebraicScheme_subscheme_affine: ...
    def _best_affine_patch(self, point: _PointLike) -> Integer: ...
    def neighborhood(self, point: _PointLike) -> AlgebraicScheme_subscheme_affine: ...
    def is_smooth(self, point: _PointLike | None = None) -> bool: ...
    def orbit(
        self, f: DynamicalSystem_projective, N: int | list[int] | tuple[int, int]
    ) -> list[Element]: ...
    def nth_iterate(
        self, f: DynamicalSystem_projective, n: int
    ) -> AlgebraicScheme_subscheme_projective: ...
    def degree(self) -> Integer: ...
    def intersection_multiplicity(
        self, X: AlgebraicScheme_subscheme_projective, P: _PointLike
    ) -> Integer: ...
    def multiplicity(self, P: _PointLike) -> Integer: ...
    def codimension(self) -> Integer: ...
    def defining_polynomials(self) -> tuple[Polynomial, ...]: ...
    def defining_ideal(self) -> MPolynomialIdeal: ...
    def Jacobian(self) -> MPolynomialIdeal: ...
    def is_irreducible(self) -> bool: ...
    def is_empty(self) -> bool: ...
    def intersection(
        self, other: AlgebraicScheme_subscheme_projective
    ) -> AlgebraicScheme_subscheme_projective: ...
    def hilbert_polynomial(self) -> Polynomial: ...
    def arithmetic_genus(self) -> Integer: ...
    def base_ring(self) -> Ring: ...
    def ambient_space(self) -> ProjectiveSpace_field: ...

class AlgebraicScheme_subscheme_projective_field(
    AlgebraicScheme_subscheme_projective
): ...
