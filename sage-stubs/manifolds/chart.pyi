...
from sage.manifolds.chart_func import (
    ChartFunction,
    ChartFunctionRing,
    MultiCoordFunction,
)
...
type _Period = (
    Element | Expression | int | float | complex | tuple[_Scalar, _Scalar] | None
)
type _Coordinates = tuple[Expression, ...]
type _RestrictionClause = (
    Expression | Sequence[Expression] | tuple[Expression, ...] | frozenset[Expression]
)
type _RestrictionInput = (
    _RestrictionClause | Callable[..., _RestrictionClause] | None
)
type _Parameters = Mapping[Expression, _Scalar]
type _Substitutions = Mapping[Expression, _Scalar]
type _CoordinateTransformations = Expression | Sequence[Expression]

class _VectorSpaceCodomain(Protocol):
    def dimension(self) -> int: ...

type _ChartCodomain = _VectorSpaceCodomain | ConditionSet

class _CoordinatePoint(Protocol):
    def coord(self, chart: Chart) -> tuple[_Scalar, ...]: ...

class _CoordinateSubset(Protocol):
    def __contains__(self, coordinate_vector: object) -> bool: ...

class CoordChange(SageObject):
    def __init__(
        self, chart1: Chart, chart2: Chart, *transformations: Expression
    ) -> None: ...
...
class Chart:
    ...
    def _restrict_set(
        self, universe: _VectorSpaceCodomain, coord_restrictions: _RestrictionClause
    ) -> _ChartCodomain: ...
    def transition_map(
        self,
        other: Chart,
