...
| T02.3 | **Integer / rational base** — audit & complete `integer`, `integer_ring`, `integer_fake`, `rational`, `rational_field`, `factorint`, `factorint_flint`, `factorint_pari`, `bernoulli_mod_p`, `bernmm` (root), `numbers_abc`, `fast_arith`. | ~12 | — | 🟡 In Progress | Most files have existing stubs; audit Phase-1 / Phase-2 of AGENTS.md. |
