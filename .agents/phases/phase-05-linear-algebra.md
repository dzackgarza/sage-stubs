...
|------|-----------------|-------|---------|--------|-------|
| T05.1 | **Matrix base & spaces** — audit & complete `matrix`, `matrix2`, `matrix_space`, `constructor`, plus root files (`berlekamp_massey`, `args`, `change_ring`, `compute_J_ideal`, `docs`, `echelon_matrix`, `matrix_misc`). | ~11 | — | 🟡 | `matrix_space` source parity landed; the remaining base files are under audit. |
| T05.2 | **Matrix abstract bases** — `matrix0`, `matrix1`, `matrix_dense`, `matrix_sparse`, `matrix_generic_dense`, `matrix_generic_sparse`. | 6 | T05.1 | 🟡 | `matrix_dense`, `matrix_sparse`, and their generic implementations are source-grounded; `matrix0` and `matrix1` remain. |
